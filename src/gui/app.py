...
class CompilerGUI(tk.Tk):
    ...
    def _crear_widgets(self):
        ...
        self.tree_packages = ttk.Treeview(self.sidebar, show='tree', selectmode="browse"); self.tree_packages.pack(fill=tk.BOTH, expand=True, padx=2, pady=5)
        ...
        self.editor_frame = tk.Frame(main_container, bg=self.bg_color); self.editor_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=2)
        ...
        self.bottom_frame = tk.Frame(self, bg=self.bg_color, height=220); self.bottom_frame.pack(fill=tk.X, side=tk.BOTTOM)
        self.lbl_error = tk.Label(self.bottom_frame, text="", bg=self.bg_color, fg=self.error_color, font=('Consolas', 10), pady=2); self.lbl_error.pack(fill=tk.X)
        self.notebook_console = ttk.Notebook(self.bottom_frame); self.notebook_console.pack(fill=tk.BOTH, expand=True)
        ...
        term_input_frame = tk.Frame(self.tab_terminal, bg=self.panel_bg); term_input_frame.pack(fill=tk.X, side=tk.BOTTOM, padx=5, pady=2)
        tk.Label(term_input_frame, text=" > ", bg=self.panel_bg, fg=self.accent_color, font=('Consolas', 12, 'bold')).pack(side=tk.LEFT)
        self.terminal_input = tk.Entry(term_input_frame, bg=self.panel_bg, fg="white", insertbackground="white", font=('Consolas', 12), borderwidth=0); self.terminal_input.pack(fill=tk.X, side=tk.LEFT, expand=True); self.terminal_input.bind("<Return>", self._on_terminal_enter); self.terminal_input.config(state='disabled')
    ...
    def add_new_editor_tab(self, file_path=None, content=""):
        ...
        txt_container = tk.Frame(tab_frame, bg=self.bg_color); txt_container.pack(fill=tk.BOTH, expand=True)
        linenumbers = LineNumbers(txt_container, width=35, bg=self.line_num_bg, highlightthickness=0); linenumbers.pack(side=tk.LEFT, fill=tk.Y)
    ...
    def get_current_editor_info(self):
        sel = self.editor_notebook.select(); return self.editors.get(sel)

    def _on_editor_change(self, tid):
        info = self.editors.get(tid)
        if info and not info['dirty']:
            info['dirty'] = True; self.editor_notebook.tab(tid, text=f"• {info['title']}")
        self._highlight_syntax(info['editor']); info['linenumbers'].redraw()
    ...
    def _on_sidebar_double_click(self, e):
        ...
        for tid, info in self.editors.items():
            if info['path'] == path: self.editor_notebook.select(tid); return
    ...
    def analizar_codigo(self):
        ...
        editor = info['editor']; codigo = editor.get("1.0", tk.END)
    ...
    def ejecutar_codigo(self):
        ...
        self.notebook_console.select(self.tab_terminal); self._append_to_terminal("🚀 Iniciando ejecución...\n\n")
        def run():
            try:
    ...
    def _request_input(self, n, vt):
        self.after(0, lambda: (self.notebook_console.select(self.tab_terminal), self._append_to_terminal(f"📥 {n} ({vt}): ", "#569CD6"), self.terminal_input.config(state='normal'), self.terminal_input.focus_set())); return self.input_queue.get()
